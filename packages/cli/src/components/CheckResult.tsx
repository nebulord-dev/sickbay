...
import type { CheckResult as CheckResultType } from '@vitals/core';
  ...
  return (
    <Box flexDirection="column" marginBottom={1}>
      ...
      <Box marginLeft={2}>
        ...
        <Text color={statusColor}> {STATUS_ICONS[result.status]} {result.status}</Text>
      </Box>
      {result.issues.slice(0, 3).map((issue, i) => (
        <Box key={i} marginLeft={2}>
          <Text color={issue.severity === 'critical' ? 'red' : issue.severity === 'warning' ? 'yellow' : 'gray'}>
