    ...
    <div className="flex h-screen overflow-hidden">
      ...
      <aside className="w-72 border-r border-border flex flex-col shrink-0 bg-surface">
        <div className="p-4 border-b border-border">
          <div className="text-green-400 font-bold text-xl tracking-wider">VITALS</div>
          <div className="text-gray-400 text-xs mt-0.5">Project Health Dashboard</div>
        </div>

        <div className="p-4 border-b border-border">
          <div className="text-xs text-gray-500 mb-1">project</div>
          <div className="font-semibold">{report.projectInfo.name}</div>
          <div className="text-xs text-gray-500">v{report.projectInfo.version}</div>
          <div className="text-xs text-gray-500 mt-1">{report.projectInfo.framework} · {report.projectInfo.packageManager}</div>
          <div className="text-xs text-gray-500">{report.projectInfo.totalDependencies} deps</div>
        </div>
          ...
          {complexityMeta.totalFiles != null && (
            <>
              <div>
                ...
                <div className="text-sm font-mono font-semibold">{fmt(complexityMeta.totalFiles as number)}</div>
              </div>
              <div>
                <div className="text-xs text-gray-500">lines</div>
                <div className="text-sm font-mono font-semibold">{fmt(complexityMeta.totalLines as number)}</div>
              </div>
            </>
          )}
          {gitMeta.contributorCount != null && (
            <>
              <div>
                ...
                <div className="text-sm font-mono font-semibold">{String(gitMeta.contributorCount)}</div>
              </div>
              <div>
                <div className="text-xs text-gray-500">last commit</div>
                <div className="text-sm font-mono font-semibold truncate">{String(gitMeta.lastCommit)}</div>
              </div>
            </>
          )}
          {coverageMeta.lines != null && (
            <div className="col-span-2">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-mono font-semibold">{Math.round(coverageMeta.lines as number)}%</span>
              </div>
            </div>
          )}
        </div>
        ...
        <nav className="p-2 flex-1 overflow-y-auto">
          ...
          {report.checks.map((check) => {
            const color = check.score >= 80 ? 'text-green-400' : check.score >= 60 ? 'text-yellow-400' : 'text-red-400';
            return (
              <button
                key={check.id}
                onClick={() => { setSelectedCheck(selectedCheck === check.id ? null : check.id); setView('overview'); }}
      ...
      <main className="flex-1 overflow-y-auto">
        ...
        <div className="p-6">
          ...
          {view === 'issues' && (
            <IssuesList checks={filteredChecks} />
          )}

          {view === 'dependencies' && (
            <DependencyList report={report} />
          )}

          {view === 'codebase' && (
            <CodebaseStats report={report} />
          )}
