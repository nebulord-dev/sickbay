    ...
    try {
      ...
      return {
        ...
        metadata: {
          ...
          inlineObjects: findings.filter((f) => f.pattern.includes('Inline')).length,
          indexAsKey: findings.filter((f) => f.pattern.includes('index as key')).length,
          largeComponents: findings.filter((f) => f.pattern.includes('Large component')).length,
        },
      };
    } catch (err) {
      return {
        ...
        score: 0,
        status: 'fail',
        issues: [{ severity: 'critical', message: `React perf check failed: ${err}`, reportedBy: ['react-perf'] }],
        toolsUsed: ['react-perf'],
        duration: elapsed(),
      };
    }
