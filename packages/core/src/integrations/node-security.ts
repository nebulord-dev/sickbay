  ...
  async run(projectPath: string): Promise<CheckResult> {
    ...
    if (hasHelmet) {
      ...
      issues.push({
        severity: 'critical',
        message:
          'Missing security headers middleware (helmet). HTTP security headers protect against XSS, clickjacking, MIME sniffing, and other common attacks.',
        fix: {
    ...
    if (hasCors) {
      ...
      issues.push({
        severity: 'warning',
        message:
          'Missing CORS middleware. Without explicit CORS configuration your API may be inaccessible from browser clients or accept requests from any origin.',
        fix: {
    ...
    if (hasRateLimit) {
      ...
      issues.push({
        severity: 'warning',
        message:
          'Missing rate limiting middleware. Without rate limiting your API is vulnerable to DoS attacks and credential brute-forcing.',
        fix: {
