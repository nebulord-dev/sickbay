  ...
  return (
    <Box flexDirection="column">
      <Box marginTop={1}>
        ...
        <Text
          color={
            diff.scoreDelta > 0 ? "green" : diff.scoreDelta < 0 ? "red" : "gray"
          }
          bold
        >
      ...
      <Box marginTop={1} marginLeft={2}>
        <Text dimColor>
          {diff.summary.improved > 0 && (
            <Text color="green">{diff.summary.improved} improved</Text>
          )}
          ...
          {diff.summary.unchanged > 0 && (
            <Text>{diff.summary.unchanged} unchanged</Text>
          )}
          {diff.summary.newChecks > 0 && (
            <Text color="cyan">, {diff.summary.newChecks} new</Text>
          )}
...
export function DiffApp({
  ...
  const [phase, setPhase] = useState<
    "scanning" | "loading-base" | "results" | "error"
  >("scanning");
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        // Save current report
        try {
          const { saveEntry, saveLastReport } = await import(
            "../lib/history.js"
          );
