...
describe('QuickWinsPanel', () => {
  ...
  it("shows 'Looking good!' when there are no fixable issues", () => {
    const report = createMockReport([
      createCheckWithIssues([{ severity: 'info', message: 'No fix here', reportedBy: ['test'] }]),
    ]);
  ...
  it('accepts availableWidth prop without error', () => {
    ...
    const { lastFrame } = render(<QuickWinsPanel report={report} availableWidth={40} />);
  ...
  it('truncates long description containing a deep file path', () => {
    ...
    const { lastFrame } = render(<QuickWinsPanel report={report} availableWidth={20} />);
