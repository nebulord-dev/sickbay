...
describe("ActivityPanel", () => {
  it("shows empty state when no entries", () => {
    const { lastFrame } = render(
      <ActivityPanel entries={[]} availableHeight={10} />
    );
  ...
  it("shows message for a single entry", () => {
    ...
    const { lastFrame } = render(
      <ActivityPanel entries={entries} availableHeight={10} />
    );
  ...
  it("limits visible entries to availableHeight", () => {
    ...
    const { lastFrame } = render(
      <ActivityPanel entries={entries} availableHeight={2} />
    );
  ...
  it("shows all entries when count is within availableHeight", () => {
    const entries = [
      makeEntry("scan-start", "Started"),
      makeEntry("scan-complete", "Completed"),
    ];
    const { lastFrame } = render(
      <ActivityPanel entries={entries} availableHeight={5} />
    );
    const output = lastFrame() ?? "";
  ...
  it("shows the most recent entries when height-limited", () => {
    const entries = Array.from({ length: 10 }, (_, i) =>
      makeEntry("info", `Message ${i + 1}`)
    );
    const { lastFrame } = render(
      <ActivityPanel entries={entries} availableHeight={3} />
    );
    const output = lastFrame() ?? "";
  ...
  it("availableHeight=1 shows only the last entry", () => {
    const entries = [
      ...
      makeEntry("scan-complete", "Last"),
    ];
    const { lastFrame } = render(
      <ActivityPanel entries={entries} availableHeight={1} />
    );
    const output = lastFrame() ?? "";
