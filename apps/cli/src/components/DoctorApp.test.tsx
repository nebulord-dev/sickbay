  ...
  it('shows fixDescription when there is no fixCommand', async () => {
    mockRunDiagnostics.mockResolvedValue([
      makeResult({
        status: 'warn',
        message: 'No engines',
        fixDescription: 'Add engines field to package.json',
      }),
    ]);
  ...
  describe('monorepo mode', () => {
    ...
    it('shows Monorepo Setup Diagnosis heading with per-package results', async () => {
      mockRunDiagnostics
        .mockResolvedValueOnce([
          makeResult({ id: 'git-a', label: 'Gitignore', status: 'pass', message: 'OK' }),
        ])
        .mockResolvedValueOnce([
          makeResult({ id: 'git-b', label: 'Gitignore', status: 'fail', message: 'Missing' }),
        ]);
    ...
    it('shows aggregate summary counts across packages', async () => {
      mockRunDiagnostics
        ...
        .mockResolvedValueOnce([makeResult({ id: 'b1', status: 'warn', message: 'w' })]);
