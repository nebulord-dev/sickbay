...
async function checkEnginesField(projectPath: string): Promise<DiagnosticResult> {
...
async function checkBrowserslist(projectPath: string): Promise<DiagnosticResult> {
  const pkg = JSON.parse(readFileSync(join(projectPath, 'package.json'), 'utf-8'));
  const hasFile =
...
async function checkNodeVersion(projectPath: string): Promise<DiagnosticResult> {
...
async function checkLockfile(projectPath: string): Promise<DiagnosticResult> {
  ...
  const hasBunLock =
    existsSync(join(projectPath, 'bun.lockb')) || existsSync(join(projectPath, 'bun.lock'));
...
async function checkEditorconfig(projectPath: string): Promise<DiagnosticResult> {
...
async function checkESLintConfig(projectPath: string): Promise<DiagnosticResult> {
  const info = await detectProject(projectPath);
  if (!info.hasESLint) {
    return {
      id: 'eslint-config',
...
async function checkReactVersions(projectPath: string): Promise<DiagnosticResult> {
  const pkg = JSON.parse(readFileSync(join(projectPath, 'package.json'), 'utf-8'));
...
export async function runDiagnostics(projectPath: string): Promise<DiagnosticResult[]> {
