...
- `[Testing]` Next.js fixture + 6 framework-scoped runners — `fixtures/packages/next-app` added with intentional issues; 6 Next.js runners (`next-images`, `next-link`, `next-fonts`, `next-missing-boundaries`, `next-security-headers`, `next-client-components`) scoped to `next` framework; snapshot regression tests; 517 core tests passing; TypeScript runner now includes line numbers in issue messages to prevent React duplicate key warnings
- `[Testing]` Angular fixture + framework-scoped runners — `fixtures/packages/angular-app` added with intentional issues (missing lazy routes, disabled strict mode, manual change detection, unmanaged subscriptions); 4 Angular-specific runners (`angular-change-detection`, `angular-lazy-routes`, `angular-strict`, `angular-subscriptions`) scoped to Angular framework; snapshot tests for angular-app fixture; full build + 457 core tests + 129 web tests passing; all 4 angular checks produce warnings on angular-app, absent from react-app