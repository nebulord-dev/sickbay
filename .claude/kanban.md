...
- `[Task]` Scan the sickbay project with the sickbay CLI — run `sickbay` against this monorepo and review the results; use findings to identify gaps and inform future work

### Features

- `[Feature]` Claude Code skill for Sickbay — a static, evergreen skill file covering: where the report lives (`.sickbay/last-report.json`), the JSON schema shape (`SickbayReport`, `CheckResult`, `Issue`), score interpretation (0–100, 80+ green, 60–79 yellow, <60 red), severity levels (critical/warning/info), and how to act on fix suggestions (`issue.fix.command`); content is deliberately high-level so it never goes stale; ship as a doc (`docs/claude-skill.md`) and optionally as `sickbay claude` command that writes it to `.claude/skills/sickbay.md` in the target project
...
- `[Feature]` Lighthouse Integration — run Lighthouse audits for Web Sickbay (LCP, FID, CLS) alongside code health checks with unified performance scoring
...
- `[Feature]` AI quick fixes — let Claude suggest and apply one-click fixes for unused imports, outdated deps, missing docs; maybe a `sickbay --fix` command
- `[Quality]` Audit `sickbay fix` executable commands — review every `fix.command` across all runners and verify each one is genuinely auto-applicable (installs a package, generates a config file, etc.) vs. guidance-only (requires human judgment, can't be automated, or could cause false-positive damage); strip `command` from any that are guidance-only so they still appear in Quick Wins but are excluded from `sickbay fix`; known issues already fixed: coverage "Fix failing tests" and "Add tests to improve coverage"; candidates to audit: knip unused exports/deps (false positive risk), depcheck missing deps, npm-audit vulnerabilities (auto-fix can break things), secrets (can't automate removal), complexity (requires refactor)
...
- `[Feature]` Add a sickbay-py variant to the CLI — Python equivalent scanning using same terminal and web views; this task IS polyglot work and should not be started until the polyglot architecture is established in `core` — building it beforehand risks creating a parallel codebase that has to be painfully retrofitted later; the right approach is to implement Python checks as first-class runners within the existing architecture once language detection and multi-language runner registration are in place
- `[Feature]` Expand the command suite — `trends` and `doctor` already exist; brainstorm and build additional commands in the same spirit that surface quick, focused visualizations or workflows without running a full scan; confirmed command ideas:
  - `sickbay envdiff` — compares `.env.example` vs actual `.env` vs deployed environment variables; flags missing, extra, or mismatched keys across environments
  - `sickbay phantomdep` — detects packages used transitively but never explicitly declared, and unused explicit deps; stricter than depcheck
  - `sickbay lockdiff` — lockfile diffing tool; human-readable summaries of what actually changed between lockfile versions ("3 deps upgraded, 1 new transitive, 1 removed")
  - `sickbay depgrave` — finds abandoned/unmaintained dependencies (last release > N months, zero recent commits, archived repos) before they become your problem
...
- `[Feature]` Sickbay badge — `sickbay badge` generates a shields.io-style markdown/HTML badge showing the project health score, color-coded green/yellow/red; badge could be static (generated once) or dynamic (served via a small endpoint); great for open source README visibility
- `[Feature]` Project config (`sickbay.config.ts`) and `.sickbay/` data folder — allow projects to calibrate check thresholds to their own standards (e.g. raise the max lines-per-file from 300 to 800 for a codebase with established conventions); scores reflect the project's health against its declared standards, with a visible "custom thresholds in effect" notice in TUI and web; config lives at root as `sickbay.config.ts` (TypeScript, `defineConfig` helper, executed via `jiti`); generated project data lives in `.sickbay/` (history, cache, baseline); `sickbay init` scaffolds both; web dashboard gets a Config tab showing active vs default values; see `docs/plans/sickbay-config-design.md` for full design; do not implement before monorepo and polyglot work is complete — the config schema will need per-package overrides in Phase 3 and building a single-project schema now means extending it carefully later
- `[Feature]` False positive suppression — allow users to mark specific check findings as intentional/irrelevant so they stop appearing in results (e.g. a helmet-less Express app behind a proxy, a "secret" that's actually a public key, an async handler pattern the scanner can't distinguish from an unprotected one); natural home is `.sickbayrc` — a `suppress` key per check id; do not implement before `.sickbayrc` lands, as the suppression config needs to live somewhere stable
- `[Feature]` Multi-repo team dashboard — `sickbay team --repos ./repos.json` scans multiple repos and shows an aggregated web dashboard with per-repo health scores, a team-level rollup, and the ability to drill into any individual project; ideal for agencies or platform teams managing many codebases; do not implement before monorepo detection lands — how repos are discovered and scanned will change once monorepo support exists, and the dashboard will need per-package drill-down within a monorepo, not just per-repo
...
- `[Feature]` Branch diff — `sickbay diff main` (or `sickbay diff <branch>`) compares health of the current branch against another; shows which checks regressed, which improved, and the overall delta; different from trend (historical over time) — this is branch-aware and perfect as a pre-push check or auto-comment on PRs
...
- `[Feature]` Monorepo-aware TUI — add `--package` flag to `sickbay tui`; without it, show a package picker or aggregate dashboard at monorepo roots
- `[Feature]` "Explain this" in the TUI — press `e` on a selected check to open a Claude-powered overlay explaining the issue and how to fix it; needs check selection, overlay component, streaming output; requires `ANTHROPIC_API_KEY`
- `[UI/UX]` TUI live score feedback — when a background re-scan completes and a score changes, the relevant panel border briefly pulses red (regression) or green (improvement)
- `[Feature]` TUI issue drill-down with file preview — press Enter on a selected check to open a half-screen overlay showing file path and relevant lines with ANSI syntax highlighting; needs: file+line metadata on check results, file-excerpt renderer, overlay with scroll
- `[Feature]` TUI command palette — press `:` to open a vim-style input bar; supports commands like `scan`, `export`, `open web`, `compare <branch>`, `theme <name>`
- `[UI/UX]` TUI mouse support — click panel to focus, scroll wheel in Health and Activity panels; Ink has mouse support via `useInput` — needs investigation
- `[Feature]` TUI resizable panels — `[` / `]` to shrink/grow focused panel; persist layout to `.sickbayrc`; blocked by config support
- `[Feature]` TUI diff mode — `sickbay tui --diff <branch>` shows two score columns (current vs branch) with delta indicators per check
### Testing

- `[Testing]` Extract fixtures to a dedicated private `sickbay-test-fixtures` repo — **primary driver is security compliance**: `node-api` contains intentional hardcoded secrets and vulnerable dependencies that will fail GitHub Enterprise security scanning and block Sickbay from reaching any internal main branch or environment; a private repo is not subject to the same scanning gates; secondary benefits: (1) CI clones the fixtures repo on demand instead of bundling it; (2) fixtures can have their own permissive security policy since issues are intentional by design; (3) versioned independently — sickbay CI can pin to a specific fixtures tag; migration path is clean since `fixtures/` is already a separate pnpm workspace — extract the directory, `git init`, push to new private repo, update CI workflow to clone it, remove `fixtures/` from the sickbay workspace; **this should be done before Sickbay is pushed to any internal infrastructure**
...
- `[Docs]` Document patterns for AI consistency — scan monorepo for code patterns and document them for AI working on Sickbay
...
- `[Feature]` Auto-increment Sickbay CLI version — set up automated version bumping (e.g. via `changesets` or a release script); display the current version number beneath the Sickbay banner in the terminal UI
...
- `[Feature]` Update notifications and `sickbay update` command — on startup, non-blocking check against the registry for a newer version; if found, print a notice under the banner (e.g. `v0.2.0 available — run "sickbay update" to upgrade`); also add a `sickbay update` command that runs the global reinstall automatically

### Code Quality & CI

- `[Quality]` Add quality checks to repo PRs — run sickbay checks as part of PR CI; no GitHub Actions workflows exist yet
...
- `[Feature]` Themeable personality quotes — configurable quote theme via `sickbay.config.ts` (`theme: "sickbay-theme-lotr"`); built-in themes: `startrek` (default), `lotr`, `stoic`, `vanilla` (plain messaging, no personality); plugin contract: theme packages export a JSON file with quotes grouped by severity key (`info`, `warning`, `critical`, `allClear`), each entry has `quote` and `source` fields; sickbay resolves the theme package via `require.resolve()` at runtime, validates shape with zod, randomly selects from the matching severity pool, falls back to `startrek` if resolution fails; community-extensible via `sickbay-theme-*` npm naming convention — anyone can publish a theme in ~20 minutes; example themes the community might build: `sickbay-theme-office`, `sickbay-theme-arrested-development`, `sickbay-theme-hitchhiker`; **per-user quote customization** — two keys in `sickbay.config.ts`: `quotes.extend` (adds user quotes to the pool alongside built-ins, randomly selected together) and `quotes.override` (replaces a severity tier entirely with user-defined quotes); resolution order: load base theme → apply `override` (full tier replacement) → apply `extend` (merge into tier) → random select from final pool; allows personalizing any built-in or third-party theme without forking or publishing a package
- `[Feature]` Star Trek doctor rotating quotes (default theme) — severity-tiered quote pool pulled from all Trek series doctors (McCoy, Crusher, The Doctor, Bashir, Phlox, etc.); quotes tagged by severity level: info (mild/cantankerous), warning (concerned), critical (dire), all-clear (rare optimism); randomly selected at render time from the matching tier; stored in a simple JSON file for easy extension; example: info → "I'm a doctor, not a DevOps engineer.", critical → "He's dead, Jim.", all-clear → "Vital signs are stable. For now."

## In Progress

- `[Task]` Rename Sickbay to `sickbay` — `sickbay` is taken on npm, `steth` was the working name but `sickbay` is the final name; update package names, CLI command, imports, branding, README, and registry config (`@nebulord/sickbay`); publish flat alias `sickbay` on npm that re-exports `@nebulord/sickbay`
