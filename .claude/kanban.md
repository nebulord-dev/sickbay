...
- `[Feature]` Expand the command suite — `trends` and `doctor` already exist; brainstorm and build additional commands in the same spirit (e.g. `sickbay compare`, `sickbay explain`, `sickbay export`) that surface quick, focused visualizations or workflows without running a full scan
